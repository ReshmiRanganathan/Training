...
SELECT s.student_name, e.course_name FROM students s LEFT JOIN enrollments e ON s.student_id = e.student_id WHERE s.city = 'Hyderabad';
SELECT * FROM enrollments WHERE fee > 6000;
...
SELECT city, COUNT(student_id) AS total_students FROM students GROUP BY city HAVING COUNT(student_id) > 1; 
